...
import { updateEmployeeServerFn } from "@/lib/employees/employees.function";

export const Route = createFileRoute("/api/employees/update")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          const body = await request.json();
          const result = await updateEmployeeServerFn({ data: body.data ?? body, request } as any);
