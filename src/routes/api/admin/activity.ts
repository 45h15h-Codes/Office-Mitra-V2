...
import { productivityLogs } from "../../../../db/schema";
      ...
      GET: async ({ request }) => {
        try {
          ...
          const allowed = await withTenantContext(session.tenantId, async (tx) => {
            return checkPermission(tx, session.userId, session.tenantId, "reports.view");
          });
          if (!allowed) {
            return new Response(
          ...
          const result = await withTenantContext(session.tenantId, async (tx) => {
            return tx
              ...
              .where(eq(productivityLogs.tenantId, session.tenantId))
              .orderBy(desc(productivityLogs.timestamp))
              .limit(100);
          });
          ...
          const message = err?.message ?? "Failed to list activity logs";
